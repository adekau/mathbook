...
def tests : TestM Unit := do
  ...
  check "exptotrig gives the square wave's partial sum" (evalText "expand(exptotrig(2i/pi*(exp(-i*t) - exp(i*t)) + 2i/(3pi)*(exp(-3i*t) - exp(3i*t))))") "4*sin(t)/π + 4/3*sin(3*t)/π"
  ...
  (st, r) := ev st "1/(1+i)"; check "Gaussian inverse" r "1/2 - 1/2*i"
  (st, r) := ev st "(2+i)/(1+i)"; check "Gaussian quotient" r "3/2 - 1/2*i"
  ...
  (st, r) := ev st "sin(3pi/4)"; check "sin 3π/4" r "1/2*sqrt(2)"
  (st, r) := ev st "tan(pi/4) + tan(pi/3)"; check "tan values" r "sqrt(3) + 1"
  (st, r) := ev st "cos(7pi/6)"; check "cos 7π/6" r "-1/2*sqrt(3)"
