...
rest of the chapter chooses the basis. The article's example is $u = (\tfrac{1}{\sqrt 2},
\tfrac{1}{\sqrt 2})$, $v = (-\tfrac{1}{\sqrt 2}, \tfrac{1}{\sqrt 2})$, $w = (0.7, 1.2)$; the
engine's radicals carry it exactly (the step $(\sqrt 2)^{2} = 2$ is \lc{simp.power}'s
$(b^m)^n = b^{mn}$ for a positive numeral base, \lc{Real.rpow_mul}).

\begin{trybox}
\begin{minted}{text}
let u = [1/sqrt(2), 1/sqrt(2)]
let v = [-1/sqrt(2), 1/sqrt(2)]
let w = [7/10, 6/5]
dot(u, v)
norm(u)
dot(u, w)
...
\end{minted}
  ...
  $\frac{19}{10\sqrt 2}$ and $\frac{1}{2\sqrt 2}$ --- the article's $1.3435$ and $0.353553$ ---
  and the last line rebuilds $w$ from them, exactly: $[\tfrac{7}{10}, \tfrac{6}{5}]$.
\end{trybox}
