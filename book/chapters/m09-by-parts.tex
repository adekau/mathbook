...
$e^x \sin x$ needs the solve-for trick. $\sin^2 x$ needs the identity $\sin^2 x = \frac{1}{2}(1 -
\cos 2x)$, which the simplifier lacks. $\frac{1}{x^2 + 1}$ needs an arctangent, which the engine
does not have. Each is refused with the candidate it tried and the derivative it got, and none is
...
\end{minted}
  ...
  fourth is the tangent. The fifth is refused, with the reason.
\end{trybox}
