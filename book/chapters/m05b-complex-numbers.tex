...
cos(7pi/6)
\end{minted}
  Type \texttt{\textbackslash e} and \texttt{\textbackslash pi} followed by a space for the constants (the notebook shows them as $\mathrm{e}$ and $\pi$). Open the work
  for the first: the power becomes an exponential, then Euler's formula fires because both values
  are exact. Click a step and see "Proof status over $\mathbb{C}$".
\end{trybox}
